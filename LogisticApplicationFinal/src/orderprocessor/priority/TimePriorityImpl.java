...
public class TimePriorityImpl implements Priority {
	...
	public Map<String, ArrayList<LogisticRecord>> processOrder(Order order, FacilityManager facilityManager,
		...
		if (order == null) {
			throw new InvalidParamException("Null Order(order) passed into TimePriorityImpl.processOrder()");
		}
		if (facilityManager == null) {
			throw new InvalidParamException(
					"Null FacilityManager(facilityManager) passed into TimePriorityImpl.processOrder()");
		}
		if (network == null) {
			throw new InvalidParamException(
					"Null NetworkProcessor(network) passed into TimePriorityImpl.processOrder()");
		}
		if (itemManager == null) {
			throw new InvalidParamException(
					"Null ItemManager(itemManager) passed into TimePriorityImpl.processOrder()");
		}
	...
	private ArrayList<LogisticRecord> generateRecords(Order order, int totalQuantity, String item)
		...
		if (facilitiesWithItemRecords.size() == 0) {
			...
			for (String facility : facilitiesWithItemRecords.keySet()) {
				...
				facilitiesWithItemRecords.get(facility).setTravelEnd(travelEnd);
			}
	...
	private void sort(ArrayList<LogisticRecord> facilityRecords) {
		Collections.sort(facilityRecords, new Comparator<LogisticRecord>() {
			...
			public int compare(LogisticRecord r1, LogisticRecord r2) {
				if (r1.getTravelEnd() > r2.getTravelEnd())
					return 1;
				if (r1.getTravelEnd() < r2.getTravelEnd())
					return -1;
				return 0;
